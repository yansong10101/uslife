...
from content.models import Feature, Permission
...
class CustomerQuerySet(models.QuerySet):
    ...
    def get_customers(self, university=None, approval_level=None):
        if university and approval_level:
            return self.filter(university=university, approval_level=approval_level, is_active=True)
        elif not university and approval_level:
            return self.filter(approval_level=approval_level, is_active=True)

    def unauthorized_user(self, university=None):
        return self.get_customers(university=university, approval_level=0)

    def authorized_user(self, university=None):
        return self.get_customers(university=university, approval_level=1)

    def alumni(self, university=None):
        return self.get_customers(university=university, approval_level=2)
...
class Customer(AbstractBaseUser):
    university = models.ForeignKey(University, related_name='customer_university', null=True)
    permission = models.ManyToManyField(Permission, related_name='customer_permission')
...
class OrgAdminQuerySet(models.QuerySet):
    ...
    def org_president(self, university=None):
        return self.filter(is_president=True, is_active=True)

    def org_admin(self):
        return self.filter(is_president=False, is_active=True)
...
class OrgAdminManager(BaseUserManager):
    ...
    def org_president(self):
        return self.get_queryset().org_president()

    def org_admin(self):
        return self.get_queryset().org_admin()
